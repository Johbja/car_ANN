...
public class AiEvolverDM : MonoBehaviour {

    [Header("Setup settings")]
    [SerializeField] private GameObject car;
    [SerializeField] private int inizialCarSpawn;
    [SerializeField] private bool useExsistingCars;
    ...
    public List<CarControllerDeathM> cars { get; private set; }
    private CarControllerDeathM carController;
    ...
    private void Start() {
        ...
        if(useImitation) {
            ...
            carController = car.GetComponent<CarControllerDeathM>();
            ...
            if(useExsistingCars) {
                ...
                cars = new List<CarControllerDeathM>();

                CarControllerDeathM[] carArr = GetComponentsInChildren<CarControllerDeathM>();
                foreach(CarControllerDeathM cc in carArr) {
                    cars.Add(cc);
                }

                inizialCarSpawn = cars.Count;


            } else {
                if(car.GetComponent<CarControllerDeathM>().isAI) {
                    cars = new List<CarControllerDeathM>();
                    car.SetActive(false);
                    ...
                    for(int i = 0; i < inizialCarSpawn; i++) {
                        ...
                        ins.SetActive(true);
                        cars.Add(ins.gameObject.GetComponent<CarControllerDeathM>());
                    }
    ...
    private CarControllerDeathM SelectCar(float fitnessSum) {
        float rand = Random.Range(0, fitnessSum);
        float counter = 0;

        foreach(CarControllerDeathM car in cars) {
            if(counter < rand) {
    ...
    private CarControllerDeathM SelectCar(int index) {
        int rand = Random.Range(0, index);
        return cars[rand];
    }

    private float[][] PointSpliceCrossover(CarControllerDeathM dominat, CarControllerDeathM submissive) {
    ...
    private float[][] UniformSpliceCrossover(CarControllerDeathM dominat, CarControllerDeathM submissive) {
    ...
    private float[] GetEvolvedNetworkGenom() {
        //temp vars
        CarControllerDeathM selectedCar1;
        CarControllerDeathM selectedCar2;
    ...
    private void QuickSort(List<CarControllerDeathM> arr, int start, int end) {
        int i;
        if(start < end) {
    ...
    private int Partition(List<CarControllerDeathM> arr, int start, int end) {
        CarControllerDeathM temp;
        CarControllerDeathM p = arr[end];
        int i = start - 1;
